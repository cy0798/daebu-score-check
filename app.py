...
# 대부고등학교 1학기 실제 나이스 엑셀 항목명 매핑 데이터 (괄호 및 오타 전수 검증 완료)
DAEBU_SUBJECTS = {
    "공통수학1": {
    ...
    "독서": {
        "독서 포트폴리오": {"만점": 20, "최저점수": 8}, 
        "주제 탐구 에세이": {"만점": 15, "최저점수": 6}, 
        "서평 쓰기 및 발표": {"만점": 15, "최저점수": 6}
    },
    "문학": {
    ...
    "과학탐구실험": {
        "실험 수행 평가": {"만점": 20, "최저점수": 8}, 
        "탐구 프로젝트": {"만점": 20, "최저점수": 8}, 
        "포트폴리오": {"만점": 10, "최저점수": 4}
    },
    "화학Ⅰ": {
        "화학 반응 실험 수행": {"만점": 25, "최저점수": 10}, 
        "원소 스토리텔링 발표": {"만점": 15, "최저점수": 6}
    },
    "화학Ⅱ": {
        "화학 평형 탐구 활동": {"만점": 30, "최저점수": 12}
    },
    "생명과학Ⅰ": {
        "세포 분열 관찰": {"만점": 20, "최저점수": 8}, 
        "유전 현상 분석": {"만점": 20, "최저점수": 8}, 
        "과학 글쓰기": {"만점": 10, "최저점수": 4}
    },
    "지구과학Ⅰ": {
        "지질 구조 탐구": {"만점": 20, "최저점수": 8}, 
        "기후 변화 발표": {"만점": 20, "최저점수": 8}, 
        "수행 포트폴리오": {"만점": 10, "최저점수": 4}
    },
    "체육": {
        "체력 측정 및 운동 계획": {"만점": 30, "최저점수": 12}, 
        "구기 경기 기능 평가": {"만점": 30, "최저점수": 12}, 
        "운동 실천 일지": {"만점": 20, "최저점수": 8}
    },
    "운동과 건강": {
        "건강 체력 평가": {"만점": 40, "최저점수": 16}, 
        "운동 처방 리포트": {"만점": 40, "최저점수": 16}
    },
    "음악": {
        "가창 수행 평가": {"만점": 30, "최저점수": 12}, 
        "기악 연주 평가": {"만점": 30, "최저점수": 12}, 
        "음악 감상 비평문": {"만점": 20, "최저점수": 8}
    },
    "미술": {
        "발상과 표현 회화 작업": {"만점": 40, "최저점수": 16}, 
        "표현 매체 탐구": {"만점": 40, "최저점수": 16}, 
    ...
    "정보": {
        "프로그래밍 실습": {"만점": 30, "최저점수": 12}, 
        "알고리즘 설계": {"만점": 30, "최저점수": 12}, 
        "정보 윤리 카드뉴스": {"만점": 20, "최저점수": 8}
    }
}
...
if uploaded_file:
    try:
        ...
        for idx in range(len(raw_df)):
            try:
                row_series = raw_df.iloc[idx].dropna()
                if row_series.empty:
                    continue
                row_values = row_series.astype(str).tolist()
        ...
        if name_col:
            for idx, row in df.iterrows():
                try:
                    ...
                    if student_name in ["nan", "", "합계", "평균", "교과", "담당", "인"]:
                        continue
