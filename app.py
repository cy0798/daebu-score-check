...
DAEBU_SUBJECTS = {
    ...
    "통합사회1": {"수업 참여 및 배움일지": {"만점": 10, "최저점수": 4}, "사회 문제 탐구 포트폴리오": {"만점": 30, "최저md": 12}},
...
if uploaded_file:
    try:
        # 나이스 엑셀 상단 공백 및 결재란 자동 건너뛰기 로직 추가
        raw_df = pd.read_excel(uploaded_file)
        
        # '성명' 또는 '이름'이 포함된 진짜 헤더 행 찾기
        header_row_idx = None
        for idx, row in raw_df.iterrows():
            row_str = "".join([str(val) for val in row.values])
            if "성명" in row_str or "이름" in row_str:
                header_row_idx = idx
                break
        
        if header_row_idx is not None:
            df = pd.read_excel(uploaded_file, skiprows=header_row_idx + 1)
            # 만약 첫 열이 멀티헤더 형태로 깨지면 보정하기 위해 컬럼 재정의
            df.columns = [str(c).strip() for c in raw_df.iloc[header_row_idx]]
        else:
            df = raw_df
        ...
        for idx, row in df.iterrows():
            # '반/번호'가 비어있거나 소계/합계 행인 경우 건너뛰기
            if pd.isna(row.get('성명')) and pd.isna(row.get('학번')):
                continue
                
            student_name = str(row.get('성명', row.get('이름', f"{idx+1}번 학생"))).strip()
            if "합계" in student_name or "교과" in student_name or student_name == "nan":
                continue
                
            student_num = row.get('반/번호', row.get('학번', '-'))

            for area, limits in subject_rules.items():
                # 엑셀 컬럼명에 줄바꿈(\n)이나 공백이 있어도 매칭되도록 보정
                target_col = next((c for c in df.columns if area.replace(" ", "") in str(c).replace(" ", "").replace("\n", "")), None)
                ...
                if target_col:
                    ...
                    if pd.isna(score) or str(score).strip() == "":
                        ...
                        if is_numeric:
                            ...
                            if clean_score not in ['결', '공', '인', '미', '45']: # 합계 수치 등 예외 방어
                                if len(clean_score) < 5: # 너무 긴 문장은 안내 제외
                                    errors.append({
                                        "반/번호": student_num, "성명": student_name, "수행평가 항목": area,
