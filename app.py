...
if uploaded_file:
    try:
        # 나이스 엑셀 파일 우선 전체 텍스트로 안전하게 읽기
        excel_data = pd.ExcelFile(uploaded_file)
        raw_df = excel_data.parse(sheet_name=0, header=None)
        
        # '성명' 이라는 단어가 포함된 진짜 헤더 인덱스 찾기 (철저한 전수조사)
        header_row_idx = None
        ...
        if name_col:
            for idx, row in df.iterrows():
                ...
                for area, limits in subject_rules.items():
                    # 엑셀의 복잡한 컬럼명에서 지정한 영역 명칭이 포함되어 있는지 유연하게 매칭
                        ...
                        if pd.isna(score) or str(score).strip() == "":
                            ...
                            if is_numeric:
                                ...
                                if clean_score not in ['결', '공', '인', '미']:
                                    # 합계 수치 등 문자열 길이가 비정상적으로 긴 통계 텍스트 제외 방어
